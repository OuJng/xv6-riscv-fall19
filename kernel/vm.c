...
void printpte(int index, pte_t pte, int level) {
...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  if(pte == 0 || (*pte & PTE_V) == 0) {
    ...
    if(mappages(p->pagetable, PGROUNDDOWN(va), PGSIZE, (uint64)newpa, PTE_W|PTE_X|PTE_R|PTE_U) != 0) {
